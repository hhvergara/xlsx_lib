...
class XLSXMerger:
    ...
    def __init__(self, file_1: str, file_2: str, output_folder_file: str, creator: str = 'MGE'):
        self.file_1 = file_1
        self.file_2 = file_2
        self.output_folder_file = output_folder_file
        self.creator = creator

    def extract_xml_from_xlsx(self, xlsx_path):
        with zipfile.ZipFile(xlsx_path, 'r') as z:
            xml_files = {name: z.read(name) for name in z.namelist()}
        return xml_files

    def get_sheet_names(self, workbook_xml):
    ...
    def get_relations(self, xml_data):
        relations = {}
    ...
    def update_relations(self, xml_data, new_rids):
    ...
    def create_new_rid(self, new_rids):
        return f"rId{len(new_rids) + 1}"

    def update_app_xml(self, sheet_names):
    ...
    def update_core_xml(self):
        ...
        ET.SubElement(root, 'cp:lastModifiedBy').text = 'lastModifiedBy'
    ...
    def write_xlsx(self, xml_data, relations, sheet_names):
        with zipfile.ZipFile(self.output_folder_file, 'w') as z:
            for filename, xml in xml_data.items():
                z.writestr(filename, xml)
            relationships_xml = self.generate_relationships_xml(relations)
            z.writestr('xl/_rels/.rels', relationships_xml)
            z.writestr('docProps/app.xml', self.update_app_xml(sheet_names))
            z.writestr('docProps/core.xml', self.update_core_xml())

    def generate_relationships_xml(self, relations):
    ...
    def merge_xlsx_files(self, a_path, b_path, output_path):
        a_xml = self.extract_xml_from_xlsx(a_path)
        b_xml = self.extract_xml_from_xlsx(b_path)
        new_rids = {}
    ...
    def descomprimir_xlsx(self, ruta_xlsx, carpeta_destino):
    ...
    def recomprimir_a_xlsx(self, carpeta_origen, ruta_salida_xlsx):
...
if __name__ == '__main__':
    ...
    merger = XLSXMerger(file_1, file_2, output_folder_file)
    merger.merge_xlsx_files()

    merger.descomprimir_xlsx(
